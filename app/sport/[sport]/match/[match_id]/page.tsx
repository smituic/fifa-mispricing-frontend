...
import { fetchMatchDetail, type Sport } from "@/lib/api";
...
export default async function MatchPage({
  ...
  params: Promise<{ sport: string; match_id: string }>;
  searchParams: Promise<{ hours?: string }>;
}) {
  const { sport: sportParam, match_id } = await params;
  ...
  const { historyData, detailData } = await fetchMatchDetail(
    match_id,
    windowHours,
    sport,
  );
  ...
  // 2-way sports (MLB, NFL, NBA, NHL) have no draw outcome. This flag drives
  // the layout: 3-way shows a center Draw card, 2-way shows a clean 2-col grid.
  const hasDraw = drawLabel !== null;
  ...
  const allCurrentOutcomes = [
    ...
    latestDraw &&
      drawLabel && { team: drawLabel, value: latestDraw.expected_value },
  ...
  function MarketDetailCard({ title, data }: { title: string; data: any }) {
    if (!data) return null;
  ...
  // Outcome grid layout: 3 columns when there's a draw, 2 columns otherwise.
  const outcomeGridClass = hasDraw
      ...
      <main className="mx-auto max-w-7xl px-6 py-8 md:px-8">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-zinc-500">
            <details className="group">
              ...
              <div className="absolute z-20 mt-3 w-[320px] rounded-2xl border border-white/8 bg-zinc-950/95 p-4 text-sm leading-6 text-zinc-400 shadow-2xl backdrop-blur">
                ...
                <div>
                  <span className="text-zinc-200">Confidence</span> measures how
                  trustworthy the edge looks.
                </div>
                <div className="mt-2">
                  <span className="text-zinc-200">Liquidity</span> measures how
                  easy the market is to trade.
                </div>
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-2 text-sm">
              {[
                ...
                <Link
                  key={option.value}
                  href={`/sport/${sport}/match/${match_id}?hours=${option.value}`}
        ...
        <div className={`mb-10 grid grid-cols-1 gap-6 ${outcomeGridClass}`}>
          <div className="rounded-3xl border border-white/8 bg-white/5 p-6 backdrop-blur">
            ...
            <div
              className={`mt-3 text-sm font-semibold ${signalTone(latestA?.expected_value)}`}
            >
            ...
            {latestA && (
              <>
                <div
                  className={`mt-4 text-4xl font-semibold ${signalTone(latestA.expected_value)}`}
                >
          ...
          {hasDraw && (
            <div className="rounded-3xl border border-white/8 bg-white/5 p-6 text-center backdrop-blur">
              ...
              {latestDraw ? (
                <>
                  <div
                    className={`mt-3 text-sm font-semibold ${signalTone(latestDraw.expected_value)}`}
                  >
                    {signalText(latestDraw.expected_value)}
                  </div>

                  <div
                    className={`mt-4 text-4xl font-semibold ${signalTone(latestDraw.expected_value)}`}
                  >
          ...
          <div className="rounded-3xl border border-white/8 bg-white/5 p-6 backdrop-blur lg:text-right">
            ...
            <div
              className={`mt-3 text-sm font-semibold ${signalTone(latestB?.expected_value)}`}
            >
            ...
            {latestB && (
              <>
                <div
                  className={`mt-4 text-4xl font-semibold ${signalTone(latestB.expected_value)}`}
                >
        ...
        {(latestA || latestDraw || latestB) && (
          <div className="mb-16">
            ...
            <div
              className={`grid grid-cols-1 gap-6 ${hasDraw ? "lg:grid-cols-3" : "lg:grid-cols-2"}`}
            >
              <MarketDetailCard title={`${teamA} Market Data`} data={latestA} />
              {hasDraw && (
                <MarketDetailCard
                  title={`${drawLabel ?? "Draw"} Market Data`}
                  data={latestDraw}
                />
              )}
