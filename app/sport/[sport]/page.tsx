...
import { isValidSport, getSport } from "@/lib/sports";
...
export default function MarketsPage({
  ...
  const stats = useMemo(() => {
    ...
    const strongest =
      [...positive].sort((a, b) => (b.top_ev || 0) - (a.top_ev || 0))[0] ??
      null;
    return { total: matches.length, positiveCount: positive.length, strongest };
  }, [matches]);
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {filteredMatches.map((match) => (
              <Link
                key={match.match_id}
                href={`/sport/${sport}/match/${match.match_id}`}
